...
{
    public class PassiveAbility_Returner : PassiveAbilityBase
    {
        public static string Name = "The Returner";
        public static string Desc = "Starts at Emotional Level 1. “Returner” triggers upon taking fatal damage, restores all HP and Stagger. “Returner” triggers up to the current emotion level. When “Returner” is triggered, max health value is increased by 20, max break value is increased by 10 and become invulnerable to status ailments. (Untransferable)";
        public override void OnWaveStart()
        {
            if (this.owner.Book.GetBookClassInfoId() == new LorId("BlackSilence", 1) || this.owner.Book.GetBookClassInfoId() == new LorId("BlackSilence", 1000000))
            {
                ...
                if (this.owner.emotionDetail.EmotionLevel <= 1)                                   //Set Emotion to 1
                {
        ...
        public override int GetMaxHpBonus()                             //Max HP:  30 + 120 = 140     When stack 1 = HP is 70
        {
            if (BattleUnitBuf_Returner.GetCharge(this.owner) >= 1)
            {
                return 20 * BattleUnitBuf_Returner.GetCharge(this.owner) + 20;
            }
            return 0;
        }
        public override int GetMaxBpBonus()                            //Max BP: 30 + 60 = 90        When stack 1 = SP is 50
        {        
          if (BattleUnitBuf_Returner.GetCharge(this.owner) >= 1)
          {
          return 10 * BattleUnitBuf_Returner.GetCharge(this.owner) + 10;
          }
          return 0;
        }
        public override bool IsImmune(KeywordBuf buf)       //Immune Seal, Nullifypower
        {
            if (Immune)
            {
        ...
        public override void OnRoundEnd()
        {
            ChangeResist();
            BattleUnitBuf activatedBuf = this.owner.bufListDetail.GetActivatedBuf(MyKeywords.Returner);
            if (activatedBuf != null && activatedBuf.stack >= 3)                 //Change Skin, Gain Adrenaline
            {
                if (!base.owner.passiveDetail.HasPassive<PassiveAbility_BlackSilenceChange>() && !base.owner.passiveDetail.HasPassiveInReady<PassiveAbility_BlackSilenceChange>())
                {
                    base.owner.passiveDetail.AddPassive(new LorId("BlackSilence", (21)));
                }
                if (!base.owner.passiveDetail.HasPassive<PassiveAbility_Adrenaline>() && !base.owner.passiveDetail.HasPassiveInReady<PassiveAbility_Adrenaline>())
                {
                    base.owner.passiveDetail.AddPassive(new LorId("BlackSilence", (7)));
                }
                if (!base.owner.passiveDetail.HasPassive<PassiveAbility_Grudge>() && !base.owner.passiveDetail.HasPassiveInReady<PassiveAbility_Grudge>())
                {
                    base.owner.passiveDetail.AddPassive(new LorId("BlackSilence", (4)));           //Gain Grudge
